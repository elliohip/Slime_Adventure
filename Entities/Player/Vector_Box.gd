...
func _draw():
	if is_touch_down:
		draw_line(player.position, Vector2((player.position.x + input_vector.x), 
		(player.position.y + input_vector.y)), Color.GREEN_YELLOW, 2.0)
...
func _input(event):
	...
	if (event is InputEventMouseMotion):
		...
		input_vector = (position_start - position_end).limit_length(player.max_acceleration)
